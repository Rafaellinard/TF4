...
class _NotificacoesScreenState extends State<NotificacoesScreen> {
  ...
  // Controladores de texto do cartão
                ...
                child: trocaPerfeita
                    ...
                    : RichText(
                        text: TextSpan(
                          ...
                          children: [
                            TextSpan(
                              text: 'Você não paga nada! ',
                              style: const TextStyle(
                                  color: Color(0xFFF59E0B), fontWeight: FontWeight.w800),
                            ),
                            TextSpan(
                              text:
                                  'A taxa de R\$ 4,99 é por conta de ${(n["solicitante"] as Map)['nome'].toString().split(' ').first}. 🎉',
                            ),
                          ],
                        ),
                      ),
          ...
          Row(children: [
            Expanded(
              ...
              child: ElevatedButton(
                ...
                child: Text(
                  trocaPerfeita ? '🎯 Aceitar — R\$ 3,99' : '✅ Aceitar — grátis para você',
  ...
  Widget _passo3Pagamento() {
    final tp = _trocaAtiva?['trocaPerfeita'] as bool? ?? false;
    final nomeOutro = (_trocaAtiva?['solicitante'] as Map?)?['nome']?.toString().split(' ').first ?? '';
    ...
    return Column(children: [
      ...
      if (tp) ...[
        Row(children: [
          for (final m in [
            ...
            Expanded(child: Padding(
              padding: EdgeInsets.only(right: m[0] == 'pix' ? 6 : 0, left: m[0] == 'cartao' ? 6 : 0),
              child: GestureDetector(
                onTap: () => setState(() => _metodoPag = m[0]),
                child: AnimatedContainer(
                  duration: const Duration(milliseconds: 150),
                  padding: const EdgeInsets.symmetric(vertical: 10),
                  decoration: BoxDecoration(
                    color: _metodoPag == m[0] ? const Color(0xFF003D1F) : const Color(0xFF161616),
                    borderRadius: BorderRadius.circular(10),
                    border: Border.all(
                      color: _metodoPag == m[0] ? const Color(0xFF00C864) : const Color(0xFF252525),
                      width: 1.5,
                    ),
                  ),
                  child: Center(child: Text(m[1],
                      style: TextStyle(
                        color: _metodoPag == m[0] ? const Color(0xFF00C864) : Colors.grey[600],
        ...
        if (_metodoPag == 'pix')
          _secBox('Pagar via Pix', [
            if (!_pixGerado)
              _btnVerde('Gerar QR Code Pix', () => setState(() => _pixGerado = true))
            else ...[
              ...
              const SizedBox(height: 8),
              Text('Escaneie com o app do seu banco', style: TextStyle(color: Colors.grey[600], fontSize: 12), textAlign: TextAlign.center),
              const SizedBox(height: 8),
              Container(
                padding: const EdgeInsets.all(10),
                decoration: BoxDecoration(
                ...
                child: const Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                  Text('Chave Pix:', style: TextStyle(color: Color(0xFF555555), fontSize: 10)),
                  SizedBox(height: 3),
                  Text('trocafigurinha2026@pagamento.com.br',
              ...
              const Text('✅ Após pagar, clique em confirmar abaixo',
                  style: TextStyle(color: Color(0xFF00C864), fontSize: 11, fontWeight: FontWeight.w700),
                  textAlign: TextAlign.center),
            ],
          ]),

        // Cartão
        if (_metodoPag == 'cartao')
          _secBox('Dados do cartão', [
            _labelW('Número do cartão'),
            _textField(_numCtrl, '0000 0000 0000 0000'),
            const SizedBox(height: 10),
            _labelW('Nome no cartão'),
            _textField(_nomeCtrl, 'NOME SOBRENOME'),
            const SizedBox(height: 10),
            Row(children: [
              Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                _labelW('Validade'), _textField(_valCtrl, 'MM/AA'),
              ])),
              const SizedBox(width: 8),
              Expanded(child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                _labelW('CVV'), _textField(_cvvCtrl, '123', obscure: true),
              ])),
            ]),
          ]),
      ],
      ...
      _loading
          ...
          : _btnVerde(
              tp ? '✅ Confirmar por R\$ 3,99' : '✅ Confirmar encontro — grátis',
              () async {
    ...
    return Center(child: SingleChildScrollView(
      ...
      child: Column(children: [
        ...
        _infoCard('💳 Pagamento', const Color(0xFF161616), const Color(0xFF252525), [
          tp ? '✅ Confirmado por R\$ 3,99 · Encontro garantido! 🎯' : '✅ Confirmado! Você não pagou nada. Apareça e troque! 🎉',
        ]),
  ...
  Widget _figCheck(Figurinha f, Set<int> set, Color cor) {
    ...
    return GestureDetector(
      ...
      child: AnimatedContainer(
        ...
        decoration: BoxDecoration(
          color: sel ? cor.withOpacity(0.15) : const Color(0xFF161616),
          borderRadius: BorderRadius.circular(8),
  ...
  Widget _btnVerde(String label, VoidCallback? onTap) => Padding(
    ...
    child: SizedBox(width: double.infinity, child: ElevatedButton(
      ...
      style: ElevatedButton.styleFrom(
        backgroundColor: const Color(0xFF00C864),
        disabledBackgroundColor: const Color(0xFF00C864).withOpacity(0.4),
  ...
  Widget _textField(TextEditingController c, String hint, {bool obscure = false}) => TextField(
    controller: c,
    obscureText: obscure,
    style: const TextStyle(color: Colors.white, fontSize: 13),
    decoration: InputDecoration(
