...
class _AppRootState extends State<AppRoot> {
  ...
  // ─── Login ────────────────────────────────────────────────────────────────
  void _onLogin(Map<String, dynamic> usuario) async {
    // Carrega figurinhas salvas do banco
    final rep = await SupabaseService.carregarRepetidas(usuario['id'] as String);
    final fal = await SupabaseService.carregarFaltam(usuario['id'] as String);
  ...
  // ─── Toggle com save automático ───────────────────────────────────────────
  void _toggleRepetida(int id) async {
    final novas = List<int>.from(_repetidas);
    if (novas.contains(id)) { novas.remove(id); } else { novas.add(id); }
    setState(() => _repetidas = novas);
    _salvarRepetidas(novas);
  }

  void _toggleFaltam(int id) async {
    final novas = List<int>.from(_faltam);
    if (novas.contains(id)) { novas.remove(id); } else { novas.add(id); }
    setState(() => _faltam = novas);
    _salvarFaltam(novas);
  }

  Future<void> _salvarRepetidas(List<int> ids) async {
    if (_usuario == null || _salvando) return;
    _salvando = true;
    await SupabaseService.salvarRepetidas(_usuario!['id'] as String, ids);
    _salvando = false;
  }

  Future<void> _salvarFaltam(List<int> ids) async {
    if (_usuario == null || _salvando) return;
    _salvando = true;
    await SupabaseService.salvarFaltam(_usuario!['id'] as String, ids);
    _salvando = false;
  }
  ...
  int get _naoLidas => _notifs.where((n) => n['status'] == 'pendente').length;
    ...
    return Scaffold(
      ...
      bottomNavigationBar: Container(
        ...
        child: BottomNavigationBar(
          ...
          selectedLabelStyle: GoogleFonts.nunito(fontWeight: FontWeight.w800, fontSize: 11),
